...
namespace MyApp
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            int nrvezes = 0;
            int k = 3;
            Console.WriteLine("tente acertar alguma letra da palavra");
            do
            {
                if (nrvezes > 0) Console.WriteLine("você errou");
                Console.WriteLine("você tem "+k+" tentativa(s)");
                char tentativa;
                tentativa = char.Parse(Console.ReadLine());
                for (i = 0; i < a; i++)
                {
                    if (tentativa == letras[i])
                    {
                        int j = i + 1;
                        Console.WriteLine("Você acertou, a letra estava na "+j+"° posição");
                        nrvezes = 3;
                    }
                }
                nrvezes++;
